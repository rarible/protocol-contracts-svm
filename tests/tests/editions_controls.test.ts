...
describe("Editions Controls Test Suite", () => {
  ...
  before(async () => {
    ...
    editionsControlsProgram = anchor.workspace
      .RaribleEditionsControls as Program<RaribleEditionsControls>;
  ...
  before(async () => {
    minter0 = Keypair.fromSecretKey(
      new Uint8Array([
        110, 76, 59, 154, 201, 225, 246, 121, 152, 90, 45, 211, 52, 244, 216, 108, 118, 248, 113,
        239, 61, 248, 207, 122, 98, 26, 184, 92, 51, 97, 52, 218, 104, 164, 83, 51, 23, 177, 193,
        29, 252, 241, 86, 132, 173, 155, 114, 131, 130, 73, 27, 101, 233, 95, 12, 45, 107, 255, 120,
        26, 121, 221, 120, 54,
      ])
    );
    minter1 = Keypair.fromSecretKey(
      new Uint8Array([
        16, 27, 49, 140, 228, 142, 201, 93, 199, 209, 62, 136, 151, 212, 238, 114, 46, 204, 155,
        132, 26, 227, 44, 245, 239, 29, 195, 63, 77, 162, 28, 220, 186, 39, 133, 92, 39, 241, 42,
        161, 180, 15, 92, 18, 15, 101, 248, 80, 238, 254, 220, 231, 1, 14, 231, 145, 170, 49, 163,
        111, 239, 112, 135, 6,
      ])
    );
    ...
    allowListConfig = {
      merkleRoot: [
        125, 184, 194, 116, 52, 36, 65, 219, 171, 135, 154, 27, 188, 122, 207, 204, 111, 70, 66,
        115, 161, 228, 44, 84, 67, 97, 29, 70, 253, 69, 11, 245,
      ],
      list: [
        {
          ...
          proof: [
            [
              64, 131, 242, 169, 206, 112, 155, 119, 81, 214, 17, 137, 174, 140, 208, 220, 141, 177,
              213, 131, 127, 104, 181, 15, 121, 228, 87, 25, 232, 172, 235, 168,
            ],
          ],
        },
        {
          ...
          proof: [
            [
              86, 37, 15, 136, 192, 159, 125, 244, 163, 213, 251, 242, 217, 215, 159, 249, 93, 166,
              82, 38, 187, 58, 199, 64, 161, 50, 122, 122, 17, 125, 63, 188,
            ],
          ],
        },
      ],
  ...
  before(async () => {
    ...
    const minter0AirdropSignature = await provider.connection.requestAirdrop(
      minter0.publicKey,
      1 * LAMPORTS_PER_SOL
    );
    ...
    const minter1AirdropSignature = await provider.connection.requestAirdrop(
      minter1.publicKey,
      1 * LAMPORTS_PER_SOL
    );
    ...
    const minter2AirdropSignature = await provider.connection.requestAirdrop(
      minter2.publicKey,
      1 * LAMPORTS_PER_SOL
    );
    ...
    const platformFeeRecipientAirdropSignature = await provider.connection.requestAirdrop(
  ...
  describe("Deploying", () => {
    it("Should deploy a collection", async () => {
      ...
      try {
        ...
        expect(editionsControlsDecoded.data.editionsDeployment.toBase58()).to.equal(
  ...
  describe.skip("Minting", () => {
    describe("Minting on a private allowlist-only phase [Phase Index 0]", () => {
      it("Should be able to mint with valid allowlist proof", async () => {
        ...
        const estimatedFee = await estimateTransactionFee(provider.connection, transaction, [
          minter0,
          mint,
          member,
        ]);
        if (VERBOSE_LOGGING) {
          console.log(
            `Estimated transaction fee: ${estimatedFee} lamports (${
              estimatedFee / LAMPORTS_PER_SOL
            } SOL)`
          );
        }
      ...
      it("Should fail to mint when exceeding allowlist max claims", async () => {
        ...
        try {
          ...
          expect(errorString).to.include(
            "This wallet has exceeded allow list max_claims in the current phase."
          );
        }
